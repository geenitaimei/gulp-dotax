  ...
  function convert(this: any, file: Vinyl, enc: any, next: Function) {
    ...
    if (file.isBuffer()) {
      ...
      workbook.forEach((sheet) => {
        ...
        // --- 修改开始：不再立即生成文件，而是追加到总字符串中 ---
        // 这里可以给每个Sheet的内容加个注释头，方便区分
        mergedKVContent += `// --- Start of Sheet: ${sheet.name} ---\n`;
        mergedKVContent += kv_data_str;
        mergedKVContent += `\n// --- End of Sheet: ${sheet.name} ---\n\n`;
        // --- 修改结束 ---
      });

      // --- 修改开始：在所有Sheet处理完后，生成一个合并文件 ---
      // 只有当有内容生成时才输出文件
...
`;
        ...
        // 修改输出文件名逻辑，避免冲突。这里使用原文件名去掉后缀加上 _merged
        const outputBasename = `${path.basename(file.path, path.extname(file.path))}_merged${kvFileExt}`;
